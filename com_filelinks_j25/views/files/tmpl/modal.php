...
<form
	action="<?php echo JRoute::_('index.php?option=com_filelinks&view=files&layout=modal&tmpl=component&function=' . $function . '&' . JSession::getFormToken() . '=1'); ?>"
			...
			<tr class="row<?php echo $i % 2; ?>">
				<td>
					...
					<?php if (file_exists(JPATH_SITE . '/' . $item->url)) : ?>
						<a class="pointer"
						   onclick="if (window.parent) window.parent.<?php echo $this->escape($function); ?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->title)); ?>','<?php echo JFactory::getApplication()->input->getString('name', 'text'); ?>');">
				...
				<td>
					<a class="pointer"
					   onclick="if (window.parent) window.parent.<?php echo $this->escape($function2); ?>('<?php echo $item->catno; ?>', '<?php echo $item->catid; ?>','<?php echo JFactory::getApplication()->input->getString('name', 'text'); ?>' );">

...
<form action="<?php echo JRoute::_('index.php?option=com_filelinks&view=files'); ?>" method="post" name="adminForm"
      id="adminForm">
...
<table class="adminlist">
	...
	<tr>
		...
		<th class='left'>
			<?php echo JHtml::_('grid.sort', 'COM_FILELINKS_FILES_URL', 'a.url', $listDirn, $listOrder); ?>
		</th>
		<th class='left'>
	...
	<?php foreach ($this->items as $i => $item) :
		...
		<tr class="row<?php echo $i % 2; ?>">
			...
			<td>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_filelinks&task=editfiledetails.edit&id=' . (int) $item->id); ?>">
			...
			<td>
				<?php
				if (file_exists(JPATH_SITE . '/' . $item->url))
				{
					if (preg_match($doctypes, $item->url))
					{
						echo $item->url;
						echo ' <a href="' . JURI::root() . $item->url . '" target="_blank">[view]</a>';
					}
					else
					{
						echo '<span class="wrongext hasTip" title="Bad file extension::File extension not allowed in options">' . $item->url . '</span>';
					}
				}
				else
				{
					echo '<span class="notfound hasTip" title="File not found::File could not be found">' . $item->url . '</span>';
				}
				?>
			</td>
			<td>
